...
void Geometry1(double xi, double eta, double & pxpxi, double & pxpeta, double & pypxi, double & pypeta,
               double & pxpxi_xi, double & pxpxi_eta, double & pxpeta_eta, double & pypxi_xi, double & pypxi_eta, double & pypeta_eta,
               double & x, double & y);
...
double forceTerm(double x, double y) {
    ...
    s6 = 9 * x * x * (3328 - 11264 * y + 11456 * y * y - 4640 * pow(y, 3) + 780 * pow(y, 4) - 60 * pow(y, 5) + 5 * pow(y,
                                                                                                                       6));
...
int main()
{
    ...
    for (int ii_y = 0; ii_y < elements_y_coupling; ii_y++)
    {
        ...
        for (int jj_y = 0; jj_y < gaussian_points; jj_y++)
        {
            ...
            Geometry2(xi, eta, pxpxi, pxpeta, pypxi, pypeta, pxpxi_xi, pxpxi_eta, pxpeta_eta, pypxi_xi, pypxi_eta, pypeta_eta, x,
                      y);
            M_C0 += weight[jj_y] * Neta_patch2 * Neta_patch2.transpose() * J_y * pow(pxpeta * pxpeta + pypeta * pypeta, .5);
            N2N1_C0 += weight[jj_y] * Neta_patch2 * Neta_patch1.transpose() * J_y * pow(pxpeta * pxpeta + pypeta * pypeta, .5);
        }
    }
    ...
    C0_constrain(0, 0) = 1;
    C0_constrain(1, 1) = 1;
    C0_constrain(dof_y_patch2 - 2, dof_y_patch1 - 2) = 1;
    C0_constrain(dof_y_patch2 - 1, dof_y_patch1 - 1) = 1;
    C0_constrain.block(2, 2, dof_y_patch2 - 4, dof_y_patch1 - 4) = M_C0.block(2, 2, dof_y_patch2 - 4,
                                                                              dof_y_patch2 - 4).partialPivLu().solve(N2N1_C0.block(2, 2, dof_y_patch2 - 4, dof_y_patch1 - 4));
    MatrixXd M_C1 = MatrixXd::Zero(dof_y_patch2, dof_y_patch2), N2N1_patch1 = MatrixXd::Zero(dof_y_patch2, dof_y_patch1), N2N2_patch1 = MatrixXd::Zero(dof_y_patch2, dof_y_patch1),
            N1N1_patch1 = MatrixXd::Zero(dof_y_patch2, dof_y_patch1), N1N1_patch2 = MatrixXd::Zero(dof_y_patch2, dof_y_patch2);
    for (int ii_y = 0; ii_y < elements_y_coupling; ii_y++)
    {
        double J_y = (knots_y_coupling[ii_y + p_y + 1] - knots_y_coupling[ii_y + p_y]) / 2;
        double Middle_y = (knots_y_coupling[ii_y + p_y + 1] + knots_y_coupling[ii_y + p_y]) / 2;
        ...
        for (int jj_y = 0; jj_y < gaussian_points; jj_y++)
        {
            ...
            double **ders_y_patch1, **ders_y_patch2;
            double nxi_xi_second_last_patch1 = DersOneBasisFUn(p_x, m_x_patch1, knots_x_patch1, m_x_patch1 - p_x - 2, xi_patch1, 1);
            double nxi_second_last_patch1 = DersOneBasisFUn(p_x, m_x_patch1, knots_x_patch1, m_x_patch1 - p_x - 2, xi_patch1, 0);
            ...
            double nxi_xi_last_patch1 = DersOneBasisFUn(p_x, m_x_patch1, knots_x_patch1, m_x_patch1 - p_x - 1, xi_patch1, 1);
            double nxi_last_patch1 = DersOneBasisFUn(p_x, m_x_patch1, knots_x_patch1, m_x_patch1 - p_x - 1, xi_patch1, 0);
            ...
            VectorXd Nxi_xiNeta_second_last_patch1 = VectorXd::Zero(dof_y_patch1),
                    ...
                    NxiNeta_second_last_patch2 = VectorXd::Zero(dof_y_patch2), Nxi_xiNeta_last_patch1 = VectorXd::Zero(dof_y_patch1),
                    NxiNeta_eta_last_patch1 = VectorXd::Zero(dof_y_patch1), Nxi_xiNeta_last_patch2 = VectorXd::Zero(dof_y_patch2),
                    NxiNeta_eta_last_patch2 = VectorXd::Zero(dof_y_patch2), NxiNeta_last_patch2 = VectorXd::Zero(dof_y_patch2), NxiNeta_last_patch1 = VectorXd::Zero(dof_y_patch1);
            for (int kk_y = 0; kk_y < p_y + 1; kk_y++)
            {
                Nxi_xiNeta_second_last_patch1(i_y_patch1 - p_y + kk_y) = ders_y_patch1[0][kk_y] * nxi_xi_second_last_patch1;
                NxiNeta_eta_second_last_patch1(i_y_patch1 - p_y + kk_y) = ders_y_patch1[1][kk_y] * nxi_second_last_patch1;
                Nxi_xiNeta_second_last_patch2(i_y_patch2 - p_y + kk_y) = ders_y_patch2[0][kk_y] * nxi_xi_second_last_patch2;
                NxiNeta_eta_second_last_patch2(i_y_patch2 - p_y + kk_y) = ders_y_patch2[1][kk_y] * nxi_second_last_patch2;
            ...
            N2N1_patch1 += beta / gamma * weight[jj_y] * NxiNeta_last_patch2 * Nxi_xiNeta_second_last_patch1.transpose() * J_y;
            ...
            N2N2_patch1 += alpha / gamma * weight[jj_y] * NxiNeta_last_patch2 * NxiNeta_eta_last_patch1.transpose() * J_y;
        }
    }
    MatrixXd C1_constrain_ker = MatrixXd(dof_y_patch2, dof_y_patch1);
    C1_constrain_ker.setZero();
    C1_constrain_ker(0, 0) = 1;
    C1_constrain_ker(1, 1) = 1;
    C1_constrain_ker(dof_y_patch2 - 2, dof_y_patch1 - 2) = 1;
    C1_constrain_ker(dof_y_patch2 - 1, dof_y_patch1 - 1) = 1;
    C1_constrain_ker.block(2, 2, dof_y_patch2 - 4, dof_y_patch1 - 4) = M_C1.block(2, 2, dof_y_patch2 - 4,
                                                                                  dof_y_patch2 - 4).partialPivLu().solve(N2N1_patch1.block(2, 2, dof_y_patch2 - 4, dof_y_patch1 - 4));
    MatrixXd C1_constrain_img = MatrixXd(dof_y_patch2, dof_y_patch1);
    C1_constrain_img.setZero();
    C1_constrain_img(0, 0) = 1;
    C1_constrain_img(1, 1) = 1;
    C1_constrain_img(dof_y_patch2 - 2, dof_y_patch1 - 2) = 1;
    C1_constrain_img(dof_y_patch2 - 1, dof_y_patch1 - 1) = 1;
    ...
    C1_constrain_img.block(2, 2, dof_y_patch2 - 4, dof_y_patch1 - 4) = M_C1.block(2, 2, dof_y_patch2 - 4,
                                                                                  dof_y_patch2 - 4).partialPivLu().solve(C1_constrain_img_rhs.block(2, 2, dof_y_patch2 - 4, dof_y_patch1 - 4));
        ...
        for (int ii_y = 0; ii_y < elements_y_patch1; ii_y++) {
            ...
            for (int jj_x = 0; jj_x < gaussian_points; jj_x++) {
                for (int jj_y = 0; jj_y < gaussian_points; jj_y++) {
                    ...
                    Hessian << pxpxi, pypxi, 0, 0, 0, pxpeta, pypeta, 0, 0, 0, pxpxi_xi, pypxi_xi, pxpxi*pxpxi, 2 * pxpxi*pypxi,
                            pypxi*pypxi, pxpxi_eta, pypxi_eta, pxpxi*pxpeta, pxpxi*pypeta + pxpeta*pypxi, pypxi*pypeta, pxpeta_eta, pypeta_eta,
                    ...
                    Nx_x_xNy = Hessian_inv(2, 0) * Nxi_xiNeta + Hessian_inv(2, 1) * NxiNeta_eta + Hessian_inv(2,
                                                                                                              2) * Nxi_xi_xiNeta + Hessian_inv(2, 3) * Nxi_xiNeta_eta + Hessian_inv(2, 4) * NxiNeta_eta_eta;
                    NxNy_y_y = Hessian_inv(4, 0) * Nxi_xiNeta + Hessian_inv(4, 1) * NxiNeta_eta + Hessian_inv(4,
                                                                                                              2) * Nxi_xi_xiNeta + Hessian_inv(4, 3) * Nxi_xiNeta_eta + Hessian_inv(4, 4) * NxiNeta_eta_eta;
                    ...
                    for (int kkx = 0; kkx < (p_x + 1) * (p_y + 1); kkx++) {
                        for (int kky = 0; kky < (p_x + 1) * (p_y + 1); kky++) {
                            ...
                            K_patch1(((m_y_patch1 - p_y) * (kkx / (p_y + 1) + i_x - p_x) + kkx %
                                                                                           (p_y + 1) + i_y - p_y), ((m_y_patch1 - p_y) * (kky /
                                                                                                                                          (p_y + 1) + i_x - p_x) + kky
                                                                                                                                                                   % (p_y + 1) + i_y - p_y)) += weight[jj_x] * weight[jj_y] * Jacobian * Bx * By * J_x * J_y;
                        }
                        F_patch1((m_y_patch1 - p_y) * (kkx / (p_y + 1) + i_x - p_x) + kkx %
                                                                                      (p_y + 1) + i_y - p_y) += weight[jj_x] * weight[jj_y] * Jacobian *
                    ...
                    for (int kkx = 0; kkx < (p_x + 1) * (p_y + 1); kkx++) {
                        for (int kky = 0; kky < (p_x + 1) * (p_y + 1); kky++) {
                            ...
                            K_patch2(((m_y_patch2 - p_y) * (kkx / (p_y + 1) + i_x - p_x) + kkx %
                                                                                           (p_y + 1) + i_y - p_y), ((m_y_patch2 - p_y) * (kky /
                                                                                                                                          (p_y + 1) + i_x - p_x) + kky
                                                                                                                                                                   % (p_y + 1) + i_y - p_y)) += weight[jj_x] * weight[jj_y] * Jacobian * Bx * By * J_x *
                                                                                                                                                                                                J_y;
                        }
    ...
    for (int i = 0; i < dof_y_patch1; i++) {
        for (int j = 0; j < dof_y_patch2; j++) {
            coefficients.push_back(Eigen::Triplet<double>(j + dof_patch1, i + dof_patch1 - dof_y_patch1, C0_constrain(j , i)));
        }
    }
    ...
    for (int i = 0; i < dof_y_patch1; i++) {
        for (int j = 0; j < dof_y_patch2; j++) {
            coefficients.push_back(Eigen::Triplet<double>(j + dof_patch1, i + dof_patch1 - 2 * dof_y_patch1, C1_constrain_ker(j ,
                                                                                                                              i)));
            coefficients.push_back(Eigen::Triplet<double>(j + dof_patch1, i + dof_patch1 - dof_y_patch1, C1_constrain_img(j , i)));
        }
    }
    ...
    SpMat transform(dof_patch1 + dof_patch2 - 2 * dof_y_patch2 - 2 * (dof_y_patch1 + dof_y_patch2) - 4 *
                                                                                                     (dof_x_patch1 + dof_x_patch2 - 2) + 16, dof_patch1 + dof_patch2 - 2 * dof_y_patch2);
